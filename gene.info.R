                       ...
                       hgVersion = c("v38", "v19")) 
{
  ...
  gentype <- function(id, data = NULL, org, hgVersion = "v38") {
    ...
    if ("ensembl_alias" %in% colnames(data)) {
      ...
      n_e_ala <- sum(id %in% data_ensembl_alias_normal | 
                       tolower(id) %in% data_ensembl_alias_lower | 
                       toupper(id) %in% data_ensembl_alias_upper)
    }
    else {
      n_e_ala <- 0L
    }
  ...
  check_web_size <- function(url) {
    web_f_size <- RCurl::getURL(url, nobody = 1L, header = 1L) %>% 
      ...
      stringi::stri_remove_empty_na() %>% as.numeric()
    return(web_f_size)
  }
  ...
  if (is.null(id)) {
    ...
    if (!is.null(id)) {
      ...
      if (length(tomany_id) > 0 & length(tomany_id) < 
          3) {
